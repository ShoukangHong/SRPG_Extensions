 ...
 * @desc Use Page Up / Page Down to cycle through valid targets for skills
 * @type boolean
 * @on YES
 * @off NO
 * @default true
 ...
 * @desc Allow the player to cycle through targets from the skill preview
 ...
 *
 * @param Auto Target
 * @desc Cursor starts on a valid target, if there is one
 * @type boolean
 * @on YES
 * @off NO
 * @default true
 *
 * @param Auto Select Self
 * @desc Skip targeting for skills that only target the user
 * @parent Auto Target
 * @type boolean
 * @on YES
 * @off NO
 * @default true
 *
 * @param Auto Select AoE
 * @desc Skip second targeting for AoE skills
 * (Requires SRPG_AreaAttack.js)
 * @parent Auto Target
 * @type boolean
 * @on YES
 * @off NO
 * @default true
 *
 * @param Auto Select Always
 * @desc Skip targeting for all skills
 * Recommended with Quick Switch From Preview
 * @parent Auto Target
 * @type boolean
 * @on YES
 * @off NO
...
(function(){
	...
	var autoTargetSelect = !!eval(parameters['Auto Select Always']);
	var autoSelfSelect = !!eval(parameters['Auto Select Self']);
	var autoAoESelect = !!eval(parameters['Auto Select AoE']);
...
//====================================================================
	...
	Game_Player.prototype.slideTo = function(x, y) {
		...
		/*this._realX = this.posX();
		this._realY = this.posY();
		this._x = x;
		this._y = y;*/
	};
...
//====================================================================
	...
	Scene_Map.prototype.selectPreviousSrpgBattleStart = function() {
		...
		if (autoTargetSelect || 
		(autoSelfSelect && $gameTemp.isSelfOnly()) ||
		(autoAoESelect && $gameTemp.isAoE())) {
			var event = $gameTemp.activeEvent();
...
//====================================================================
	...
	Scene_Map.prototype.switchMenuTarget = function(reverse) {
		...
		var next = $gameSystem.findSelection("target", current, reverse);
		SoundManager.playCursor();
	...
	Scene_Map.prototype.startActorTargetting = function() {
		...
		if (autoTarget) {
			...
			if (autoTargetSelect || (autoSelfSelect && $gameTemp.isSelfOnly())) {
				var event = $gameMap.event(id);
	...
	Game_Player.prototype.triggerAction = function() {
		...
		if (wasAoE && $gameSystem.isSubBattlePhase() === 'actor_target') {
			var id = $gameSystem.findSelection("target");
			if (autoTargetSelect || autoAoESelect) {
				var event = $gameMap.event(id);
				if (event) this.startMapEvent(event.posX(), event.posY(), "0", false);
			}
		}
	}
	...
	// check if the current skill is an AoE or not
	Game_Temp.prototype.isAoE = function() {
